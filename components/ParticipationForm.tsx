...
const ParticipationForm: React.FC<ParticipationFormProps> = ({ isOpen, onClose, onSave, participationToEdit, publishers, participations, rules, specialEvents, eventTemplates }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 1. Validação de Duplicidade de Parte (Mesmo título ou tipo único na mesma semana)
    // Tipos únicos: Presidente, Leitor, Dirigente, Orações. Não deve haver dois presidentes na mesma semana.
    const isUniquePartType = [ParticipationType.PRESIDENTE, ParticipationType.LEITOR, ParticipationType.DIRIGENTE, ParticipationType.ORACAO_INICIAL, ParticipationType.ORACAO_FINAL].includes(partType as ParticipationType);
    
    const duplicatePart = participations.find(p => 
        p.week === week && 
        p.id !== participationToEdit?.id && // Ignora a si mesmo na edição
        (isUniquePartType ? p.type === partType : p.partTitle.trim().toLowerCase() === partTitle.trim().toLowerCase())
    );

    if (duplicatePart) {
        alert(`Já existe uma parte cadastrada com este título ("${duplicatePart.partTitle}") ou tipo nesta semana. Verifique a pauta.`);
        return;
    }

    // 2. Validação de Disponibilidade/Sobrecarga do Publicador (Student)
    const studentConflicts = participations.filter(p => 
        p.week === week && 
        p.publisherName === studentName && 
        p.id !== participationToEdit?.id
    );

    if (studentConflicts.length > 0) {
        const assignedParts = studentConflicts.map(p => p.partTitle).join(', ');
        if (!window.confirm(`O publicador ${studentName} JÁ possui designação(ões) nesta semana: ${assignedParts}. \n\nIsso pode ser uma sobrecarga ou erro. Deseja continuar mesmo assim?`)) {
            return;
        }
    }
    ...
    if (isPairedPart) {
        ...
        // 3. Validação de Sobrecarga do Ajudante (Helper)
         const helperConflicts = participations.filter(p => 
            p.week === week && 
            p.publisherName === helperName && 
            p.id !== participationToEdit?.id // Difícil rastrear o ID do par editado aqui se não vier na prop, mas previne conflito básico
        );

        if (helperConflicts.length > 0) {
            const assignedParts = helperConflicts.map(p => p.partTitle).join(', ');
             if (!window.confirm(`O ajudante ${helperName} JÁ possui designação(ões) nesta semana: ${assignedParts}. \n\nDeseja continuar mesmo assim?`)) {
                return;
            }
        }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg m-4 flex flex-col" style={{maxHeight: '90vh'}} onClick={e => e.stopPropagation()}>
        <div className="p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">{participationToEdit ? 'Editar Participação' : 'Adicionar Designação'}</h2>
        </div>
        <form onSubmit={handleSubmit} id="participation-form" className="flex-grow overflow-y-auto p-6 space-y-4">
            <div>
